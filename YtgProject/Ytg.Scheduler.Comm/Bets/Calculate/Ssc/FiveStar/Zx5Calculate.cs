...
{
    /// <summary>
    /// 五星/五星组选/组选5  验证通过 2015/05/23 2016 01 17
    /// </summary>
    public class Zx5Calculate : ZhiXuanFushiDetailsCalculate
    {
        /// <summary>
        /// 计算中奖情况
        /// </summary>
        ...
        private int TotalBet(BasicModel.LotteryBasic.BetDetail betDetail, string openResult)
        {
            ...
            int winCount = 0;
            foreach (var si in erChongHao)
            {
                var danItems = danHao.Where(d => d != si).Select(d => d.ToString()).ToList();
                if (danItems.Count < 1)
                    continue;
        ...
        {
            string res = "";
            var array = string.Format("{0}{1}{1}{1}{1}", c, er).OrderBy(x => x);
