    ...
    public class RenXuanZhiXuanFushi : BaseRenXuan
    {
        protected override void IssueCalculate(string issueCode, string openResult, BasicModel.LotteryBasic.BetDetail item)
        {
            ...
            else
            {
                ...
                bool isWin = false;
                var list = GetAllBets(contentArray[0], contentArray[1], contentArray[2], contentArray[3]);
        ...
        /// </summary>
        /// <returns></returns>
        private List<string> GetAllBets(string c0, string c1, string c2, string c3)
        {
            var list = new List<string>();
            var shi = c0.Select(m => Convert.ToInt32(m.ToString())).ToList();
            var ge = c1.Select(m => Convert.ToInt32(m.ToString())).ToList();
            var three = c2.Select(m => Convert.ToInt32(m.ToString())).ToList();
            var fire = c3.Select(m => Convert.ToInt32(m.ToString())).ToList();

            list = (from s in shi
                    from g in ge
                    from t in three
                    from f in fire
                    select string.Format("{0}{1}{2}{3}", s, g, t, f)).ToList();
        ...
        public override int TotalBetCount(BasicModel.LotteryBasic.BetDetail item)
        {
            ...
            return contentArray[0].Length * contentArray[1].Length * contentArray[2].Length * contentArray[3].Length;
        }
