...
namespace Ytg.Scheduler.Comm.Bets.Calculate.Ssc.RenXuanSi
{
    ...
    /// 任选组24 2016 01 21
    /// </summary>
    public class RenXuanZu24 : BaseRenXuan
    {
        protected override void IssueCalculate(string issueCode, string openResult, BasicModel.LotteryBasic.BetDetail item)
        {
            ...
            Combinations<string> cx = new Combinations<string>(content.Replace(",","").Select(t => t.ToString()).ToList(), 4);
            ...
            {
                count = list.Count(m => winRes.Contains(string.Join("", m.OrderBy(x=>x))));
            }
            if (count > 0)
            {
                item.IsMatch = true;
