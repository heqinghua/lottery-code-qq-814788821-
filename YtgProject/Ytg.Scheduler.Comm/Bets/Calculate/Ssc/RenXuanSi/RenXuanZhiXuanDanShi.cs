...
namespace Ytg.Scheduler.Comm.Bets.Calculate.Ssc.RenXuanSi
{
    ...
    /// 直选单式 20160121
    /// </summary>
    public class RenXuanZhiXuanDanShi : BaseRenXuan
    {
        protected override void IssueCalculate(string issueCode, string openResult, BasicModel.LotteryBasic.BetDetail item)
        {
            ...
            {
                string winResult = result[b[0]-1].ToString() + result[b[1]-1].ToString() + result[b[2]-1].ToString() + result[b[3]-1].ToString();
        ...
        public override int TotalBetCount(BasicModel.LotteryBasic.BetDetail item)
        {
            ...
            if (!string.IsNullOrEmpty(content) && postionStr.Length >= 4)
            {
                ...
                int notes = 0;
                Combinations<int> v = new Combinations<int>(postionStr.Select(c => Convert.ToInt32(c)).ToList(), 4);
                foreach (var b in v._combinations)
                {
                    notes += ctLength;
                }
        ...
        public override string HtmlContentFormart(string betContent)
        {
            ...
            foreach (var item in contentArray)
            {
                ...
                if (nums.Count() != 4) return string.Empty;
