...
function AddProductProperty()
{
    try
    {
        if (CheckeProductProperty())
        {
            var productId = $("#txtPropductId").val();
            var color = $("#txtColor").val();
            var size = $("#txtSize").val();
            var spec = $("#txtSpec").val();
            var version = $("#txtVersion").val();
            var price = parseFloat($("#txtPrice").val());
            var stock = parseInt($("#txtStock1").val());

            //添加产品属性到数据库,同时将产品展示到页面
            var postData = {
                "productId": productId,
                "color": color,
                "size": size,
            ...
            $.ajax({
                type: "post",
                url: "../../tools/admin_ajax.ashx?action=AddProductProperty",
                data: postData,
                dataType: "json",
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    $.dialog.alert('尝试发送失败，错误信息：' + errorThrown, function () { }, api);
                },
                success: function (data, textStatus) {
                    if (data.status == 1) {

                        var tr = "<tr id=" + data.propertyId + ">";
                        tr = tr + "<td width=\"14%\"><label id=" + data.propertyId + "_color" + " name=\"color\" >" + color + "</label></td>";
                        tr = tr + "<td width=\"14%\"><label id=" + data.propertyId + "_size" + " name=\"size\" >" + size + "</label></td>";
                        tr = tr + "<td width=\"14%\"><label id=" + data.propertyId + "_spec" + " name=\"spec\" >" + spec + "</label></td>";
                        tr = tr + "<td width=\"14%\"><label id=" + data.propertyId + "_version" + " name=\"version\" >" + version + "</label></td>";
                        tr = tr + "<td width=\"14%\"><label id=" + data.propertyId + "_price" + " name=\"price\" >" + price + "</label></td>";
                        tr = tr + "<td width=\"14%\"><label id=" + data.propertyId + "_stock" + " name=\"stock\" >" + stock + "</label></td>";
                        tr = tr + "<td width=\"14%\"><a href=\"#\" onclick=\"EditProductProperty('" + data.propertyId + "');\">修改</a></td>";
                        tr = tr + "</tr>";
                        $("#productPropertyList").append(tr);

                        $.dialog.tips(data.msg, 1.5, '32X32/succ.png', function () { });
                    } else {
                        $.dialog.tips(data.msg, 1.5, '32X32/succ.png', function () { });
                    }
                }
...
function AddProductPropertys(form, to)
{
    try
    {
        if (CheckeProductProperty())
        {
            ...
            var trId = GetGuid().toString();
            var tr = "<tr id=" + trId + ">";
            tr = tr + "<td width=\"14%\"><label name=\"color\" >" + color + "</label></td>";
            tr = tr + "<td width=\"14%\"><label name=\"size\" >" + size + "</label></td>";
            tr = tr + "<td width=\"14%\"><label name=\"spec\" >" + spec + "</label></td>";
            tr = tr + "<td width=\"14%\"><label name=\"version\" >" + version + "</label></td>";
            tr = tr + "<td width=\"14%\"><label name=\"price\" >" + price + "</label></td>";
            tr = tr + "<td width=\"14%\"><label name=\"stock\" >" + stock + "</label></td>";
            tr = tr + "<td width=\"14%\"><a href=\"#\" onclick=\"ConfirmBox('" + trId + "');\">移除</a></td>";
            tr = tr + "</tr>";
            
            if ($("#ProductPropertyItems").val() == "") {
                $("#ProductPropertyItems").val(productPropertyItem);
            }

            $("#ProductPropertyItems").val($("#ProductPropertyItems").val() + "|" + productPropertyItem);
            $("#" + to + " tbody").append(tr);

            //var trId = GetGuid().toString();
            //var tr = "<tr id=" + trId + ">";
            //var productPropertyItem = "{";
            //$("#" + form + " tbody tr").children().each(function (i, td) {
            //    if ($(td).children().get(0).tagName.toLowerCase() == "input") {
            //        var input = $(td).children("input").get(0);
            //        if ($(input).attr("name") == "color") {
            //            productPropertyItem = productPropertyItem + "\"color\":" + "\"" + $(input).val() + "\"";
            //            tr = tr + "<td width=\"14%\"><label name=\"color\" >" + $(input).val() + "</label></td>";
            //        }
            //        if ($(input).attr("name") == "size") {
            //            productPropertyItem = productPropertyItem + ",\"size\":" + "\"" + $(input).val() + "\"";
            //            tr = tr + "<td width=\"14%\"><label name=\"size\" >" + $(input).val() + "</label></td>";
            //        }
            //        if ($(input).attr("name") == "spec") {
            //            productPropertyItem = productPropertyItem + ",\"spec\":" + "\"" + $(input).val() + "\"";
            //            tr = tr + "<td width=\"14%\"><label name=\"spec\" >" + $(input).val() + "</label></td>";
            //        }
            //        if ($(input).attr("name") == "version") {
            //            productPropertyItem = productPropertyItem + ",\"version\":" + "\"" + $(input).val() + "\"";
            //            tr = tr + "<td width=\"14%\"><label name=\"version\" >" + $(input).val() + "</label></td>";
            //        }
            //        if ($(input).attr("name") == "price") {
            //            productPropertyItem = productPropertyItem + ",\"price\":" + "\"" + $(input).val() + "\"";
            //            tr = tr + "<td width=\"14%\"><label name=\"price\" >" + $(input).val() + "</label></td>";
            //        }
            //        if ($(input).attr("name") == "stock") {
            //            productPropertyItem = productPropertyItem + ",\"stock\":" + "\"" + $(input).val() + "\"";
            //            tr = tr + "<td width=\"14%\"><label name=\"stock\" >" + $(input).val() + "</label></td>";
            //        }
            //    }
            //});
            //productPropertyItem = productPropertyItem + "}";
            //if ($("#ProductPropertyItems").val() == "") {
            //    $("#ProductPropertyItems").val(productPropertyItem);
            //}
            //$("#ProductPropertyItems").val($("#ProductPropertyItems").val() + "|" + productPropertyItem);
...
function CheckeProductProperty() {
    var color = $("#txtColor").val();
    var size = $("#txtSize").val();
    var spec = $("#txtSpec").val();
    var version = $("#txtVersion").val();
    ...
    if ($.trim(color) == "" && $.trim(size) == "" && $.trim(spec) == "" && $.trim(version) == "") {
        $.dialog.tips("请输入产品属性!", 1.5, '32X32/succ.png', function () { });
        return false;
    }

    if ($.trim($("#txtPrice").val()) == "") {
        $.dialog.tips("请输入金额!", 1.5, '32X32/succ.png', function () { });
        $("#txtPrice").focus();
        return false;
    }

    if (!/^[1-9][0-9]+(.[0-9]{1,2})?$/.test($.trim($("#txtPrice").val()))) {
        $.dialog.tips("金额格式错误!", 1.5, '32X32/succ.png', function () { });
        $("#txtPrice").focus();
        return false;
    }

    if ($.trim($("#txtStock1").val()) == "") {
        $.dialog.tips("请输入库存!", 1.5, '32X32/succ.png', function () { });
        $("#txtStock1").focus();
        return false;
    }

    if (!/^[1-9][0-9]*$/.test($.trim($("#txtStock1").val()))) {
        $.dialog.tips("库存格式错误!", 1.5, '32X32/succ.png', function () { });
        $("#txtStock1").focus();
        return false;
    }
    return true;
