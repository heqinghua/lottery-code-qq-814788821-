...
(function ($) {//start
    ...
    $.gameInit = function (opts) {//整个购彩界面的初始化
        ...
        var ps = {//整个JS的初试化默认参数
            ...
            data_id: {
                ...
                id_game_tzallmon: "game_tzallmon",//订单总金额合买
                id_game_tzallmon: "game_total_fen",//合买最低认购金额
            },
    ...
    var lt_smalllabel = function (opts) {//动态载入小标签
        ...
        $.each(opts.label, function (i, n) {
            ...
            if (n.gtitle != "")
                isShowDiv = "";
            html += '<div ><span class="methodgroupname" ' + isShowDiv + '>' + n.gtitle + '：</span>';
            $.each(n.label, function (ii, nn) {
                if (typeof (nn) == 'object') {
                    ...
                    if (i == 0 && ii == 0) {//第一个标签自动选择
                        html += '<label ' + isShowDiv + ' for="smalllabel_' + i + '_' + ii + '" id="smalllabel_' + i + '_' + ii + '" value="' + i + '-' + ii + '" class="com_btn"><span>' + nn.desc + '</span></label>&nbsp;';
...
//SELECT框就会用到，单选框就没用到了。
                        ...
                        if ((typeof (nn.nfdprize) != "undefined") && (nn.nfdprize.levs != "") && (typeof (nn.nfdprize.levs) != "undefined")) {
                            ...
                            $nfdhtml += '<option value ="2" selected="selected" >奖金' + nn.nfdprize.levs + '</option>';
                            //$("#nfdprize").html($nfdhtml);
                            $("#nfdprize").html("奖金：" + nn.nfdprize.levs);
...
//*/
                        ...
                        html += '<label for="smalllabel_' + i + '_' + ii + '" id="smalllabel_' + i + '_' + ii + '" class="com_btn_h" value="' + i + '-' + ii + '" ><span>' + nn.desc + '</span></label>&nbsp;';
                    }
                }
            });
        });
        ...
        $("#lt_samll_label label").click(function () {
            ...
            if (typeof (opts.label[index[0]].label[index[1]].nfdprize.defaultprize) != 'undefined' && opts.label[index[0]].label[index[1]].nfdprize.levs != '') {
                ...
                //$("#nfdprize").html($nfdhtml);
                $("#nfdprize").html("奖金：" + opts.label[index[0]].label[index[1]].nfdprize.levs);
                $("#wrapshow").css("display", 'block');
                //添加绑定记录用户返金模式功能,自动选择已选过的模式。
                var pmode_value = getCookie("pmode_selected_value");
                if (pmode_value != null)
    ...
    $.fn.lt_ajaxSubmit = function () {
        ...
        $(this).click(function () {
            ...
            if ($('input[@name="operate2"][checked]').val() == 1) {

                var ps = 0;
                if ($("#share_cnt").val() != "")
                    ps = parseFloat($("#share_cnt").val());
                var minm = parseFloat($("#game_total_fen").html());
                if (ps < minm) {
                    parent.$.alert("  最少购买10%，即" + minm + "元");
