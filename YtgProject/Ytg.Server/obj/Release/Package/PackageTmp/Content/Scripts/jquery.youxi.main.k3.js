...
;(function($){//start
    ...
    $.gameInit = function(opts){//整个购彩界面的初始化
		...
		//sam edit
                ...
                $($.lt_id_data.id_helpdiv).css({"left":(offset.left-$($.lt_id_data.id_helpdiv).width() + 64)+"px","top":(offset.top + 5)+"px"}).show();
        },function(){
    ...
    var lt_smalllabel = function(opts){//动态载入小标签
        ...
        $.each(opts.label, function(i,n){
				html += '<div><span class="methodgroupname">'+n.gtitle+'</span>：';
				...
				$.each(n.label, function(ii,nn){
					if(typeof(nn)=='object'){
						...
						if( i == 0 && ii == 0){//第一个标签自动选择
							...
							if($.lt_method_data.methodid == 2437)
							{
								$("#poschoose_1").show();
							} else {
								$("#poschoose_1").hide();
...
//*/
    ...
    $.fn.lt_timer = function (start, end, betend) {//服务器开始时间，服务器结束时间
        ...
        var me = this;
        if (start==undefined || start == "" || end == undefined || end == "" || betend==undefined || betend == "") {
            $.lt_time_leave = 0;
            $.lt_time_end = 0;
        } else {
        ...
        var timerno = window.setInterval(function () {
            ...
            if ($.lt_time_leave > 0 && ($.lt_time_leave % 40 == 0 || $.lt_time_leave == 30)) {//每隔30秒以及最后一分钟重新读取服务器时间
