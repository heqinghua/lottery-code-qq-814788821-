...
(function ($) {//start
    ...
    $.gameInit = function (opts) {//整个购彩界面的初始化
        ...
        var ps = {//整个JS的初试化默认参数
            ...
            data_id: {
                ...
                id_sub_subCannel: '#subCannel',//取消提交
                id_bet_money_pop: '#bet_money_pop',//取消提交
                id_comfirmsumbit: '#comfirmsumbit',//提交
                id_shot_end_div: '#shot_end_div'//封单提示
            },
            cur_issue: {issue: '20160510001', endtime: '2016-05-10 09:10:00'}, //当前期
            issues: {//所有的可追号期数集合
                today: [],
                tomorrow: []
            },
            servertime: '2016-05-10 09:09:40', //服务器时间[与服务器同步]
            lotteryid: 1, //彩种ID
            ontimeout: function () {
       ...
       // opts.ajaxurl = null;
            ...
            if (typeof (n) == 'object') {
                if (i == 0) {//第一个标签自动选择
                    if (n.isnew == 1) {
                        ...
                        bhtml += ' <li><a class="play_id_7"  value="' + i + '" href="javascript:;"  id="two' + (i + 1) + '">' + n.title + '</a></li>';
                    } else {
                    ...
                    if (n.isdefault == 1) {//选择默认标签
                        ...
                        if (n.isnew == 1) {
                            ...
                            bhtml += ' <li><a class="play_id_7 beet-active"  value="' + i + '" href="javascript:;"  id="two' + (i + 1) + '">' + n.title + '</a></li>';
                        } else {
        ...
        if (hasdefault == false) {
            $($.lt_id_data.id_labelbox + " li").eq(0).removeClass().addClass("beet-active");
        }
        ...
        $($.lt_id_data.id_labelbox).children().click(function () {//切换标签
            ...
            if ($($(this).children()[0]).attr("class").indexOf('beet-active') >= 0) {//如果已经是当前标签则不切换
                   
                return;
            }
            $(this).siblings().each(function () {
                $($(this).children()[0]).removeClass("beet-active");
            });

            $($(this).children()[0]).addClass("beet-active");

             //*/
            var index = parseInt($($(this).children()[0]).attr("value"), 10);
        ...
        $($.lt_id_data.id_cf_clear).click(function () {//清空按钮
            parent.$.confirm(lot_lang.am_s5, function () {
                ...
                $($.lt_id_data.id_cf_content).children().remove();
        ...
        $($.lt_id_data.id_comfirmsumbit).lt_ajaxSubmit();
    ...
    var lt_smalllabel = function (opts) {//动态载入小标签
        ...
        $.each(opts.label, function (i, n) {
            ...
            //html += '<div style="text-align:left;padding-left:5px;"><span class="methodgroupname" ' + isShowDiv + ' style="display:none;">&nbsp;&nbsp;' + n.gtitle + '：</span><br/>';
            html += '<div style=""><span class="methodgroupname" ' + isShowDiv + ' style="display:none;">&nbsp;&nbsp;' + n.gtitle + '：</span>';
            $.each(n.label, function (ii, nn) {
                if (typeof (nn) == 'object') {
                    ...
                    if (i == 0 && ii == 0) {//第一个标签自动选择
                        ...
                        set_check_title(n.gtitle + "-" + nn.desc,false);
                        set_check_cat_title(nn.desc);
                        ...
                        html += '<li ' + isShowDiv + '><a class="beet-active" tag=' + n.gtitle + ' id="smalllabel_' + i + '_' + ii + '" value="' + i + '-' + ii + '" style="font-size: 14px;">' + nn.desc + '</a></li>';
                        ...
                        $.each(nn.modes, function (j, m) {
                           ...
                           // console.info("m.modeid="+m.modeid+"  "+ $.lt_method_data.modes[1].rate + "rate");
                            if (is_select)
                        ...
                        if ((typeof (nn.nfdprize) != "undefined") && (nn.nfdprize.levs != "") && (typeof (nn.nfdprize.levs) != "undefined")) {
                            ...
                            $("#prize_show,#prize_true").val(nn.nfdprize.levs);
...
//*/
                        ...
                        html += '<li ' + isShowDiv + '><a  tag=' + n.gtitle + ' id="smalllabel_' + i + '_' + ii + '" value="' + i + '-' + ii + '" style="font-size: 14px;">' + nn.desc + '</a></li>';
                    }
                }
            });
        });
        ...
        $("#lt_samll_label li").click(function () {
        
            //beet-active
            var nowCd = $($(this).children()[0]);
            if (nowCd.hasClass("beet-active") == true) {//如果已经选择则无任何动作
               // return;
            }
            $("#lt_samll_label li a").removeClass("beet-active");
           // $("#lt_samll_label label").removeClass("com_btn").addClass("com_btn_h");
            nowCd.addClass("beet-active");

            var sel_title = nowCd.attr("tag") + "-" + nowCd.html();
            // set_check_title(n.gtitle + "-" + nn.desc,false);
            close_trad_detaClose(sel_title);//隐藏弹出框

            var index = nowCd.attr("value").split('-');
    ...
    $.fn.lt_timer = function (start, end, betend) {//服务器开始时间，服务器结束时间
        ...
        var timerno = window.setInterval(function () {
            ...
            var vxh = "<span id=\"time_h1\">0</span>" +
                           "<span id=\"time_h2\">0</span>" +
                           "<span class=\"time-kong\"></span>" +
                           "<span id=\"time_m1\">0</span>" +
                           "<span id=\"time_m2\">3</span>" +
                           "<span class=\"time-kong\"></span>" +
                           "<span id=\"time_s1\">0</span>" +
                           "<span id=\"time_s2\">6</span>";

            var hha = fftime(oDate.hour);
            var mma = fftime(oDate.minute);
            var mms = fftime(oDate.second);
    ...
    $.fn.lt_ajaxSubmit = function () {
        ...
        function ajaxSubmit() {
            ...
            //    var contentInfo = $(this).val();
            //    var regS = new RegExp("'", "g");
            //    contentInfo = contentInfo.replace(regS, "\"");
            //   // console.info(contentInfo);
            //    //{'type':'digital','methodid':1000,'codes':'1&3&5&7&9||||','nums':5,'omodel':2,'times':1,'money':10,'mode':1,'desc':'[定位胆_定位胆] 13579,,,,'}
            //    var jsonObj = JSON.parse(contentInfo);
            //    jsonObj.times = $("#lt_sel_times").val();
            //    $(this).val(JSON.stringify(jsonObj));
                ...
                success: function (data) {
                   parent.Ytg.common.user.refreshBalance();//刷新金额
                    ...
                    if (jsonData.Code == 0) {
                        ...
                        $($($.lt_id_data.id_sub_subCannel)).get(0).click();
                        //成功
